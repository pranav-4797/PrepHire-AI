...
import { AdminProblemsManager } from './components/coding/AdminProblemsManager'
import { analyzeResume, buildResumeContext, extractResumeText } from './utils/resume'
...
const DOMAINS: Domain[] = [
  {
    ...
    desc: 'DSA algorithms, system design, coding best practices, debugging, API design, databases, concurrency, optimization',
    questions: [
      'Explain the time complexity of binary search.',
      'Design a scalable microservice architecture.',
      'Debug a production performance bottleneck.',
      'Explain RESTful vs GraphQL design trade-offs.',
  ...
  },
  {
    ...
    label: 'HR',
    icon: <User size={18} />,
    desc: 'Behavioral questions, soft skills, communication, cultural fit, career goals',
    questions: [
  ...
  },
  {
    ...
    desc: 'Quantitative reasoning, logical problems, mental math, data interpretation',
    questions: [
      ...
      'Calculate the percentage increase from 40 to 60.',
      'Determine the missing number in the sequence: 2, 4, 8, 16, ?.',
      'Solve a word problem involving ratios and proportions.',
    ],
  },
  {
    id: 'group-discussion',
    ...
    desc: 'Group discussion topics, communication, debate, consensus building',
    questions: [
      ...
      'Evaluate the pros and cons of a four-day work week.',
      'How should companies handle employee burnout?',
    ],
  },
]
...
function _debounce<T extends (...args: unknown[]) => void>(fn: T, delay: number) {
...
function getNextQuestionIndex(domainQuestions: string[], usedIndices: number[] = []): number {
  if (domainQuestions.length === 0) return 0
  if (usedIndices.length >= domainQuestions.length) return 0
  const availableIndices = domainQuestions
    .map((_, idx) => idx)
...
function ProctoredCamera({
  ...
  const [faceStatus, setFaceStatus] = useState<'detecting' | 'detected' | 'missing'>('detecting')
  ...
  }, [active, initCamera, stopCamera, videoRef])
  ...
  useEffect(() => {
    ...
    const runDetection = async () => {
      ...
      if (!tf || !cocoSsd) return // not loaded yet
      ...
      try {
        ...
        predictions.forEach((p: { class: string }) => {
        ...
        if (personCount === 0) {
          ...
          if (Date.now() - lastFace.current > 4000) {
        ...
        if (personCount > 1) {
          addWarn('Multiple people detected in frame')
        }
        
        if (phoneDetected) {
          addWarn(`Phone detected at ${new Date().toLocaleTimeString()}`)
        }
    ...
    lastFace.current = Date.now()
    ivRef.current = setInterval(runDetection, 5000)
...
function LoginScreen() {
  ...
  async function handleSubmit() {
    ...
    if (mode === 'register' && form.role !== 'Admin' && !form.branch) {
      setErr('Please select your branch.')
      return
    }
    ...
    try {
      if (mode === 'login') {
        ...
        await register({
          ...
          role: form.role,
          department: form.role === 'Admin' ? 'General' : form.branch,
        })
      }
    } catch (error) {
  ...
  return (
    <div
      style={{
        minHeight: '100vh',
        backgroundImage: 'linear-gradient(to bottom, rgba(7, 19, 39, 0.55), rgba(7, 19, 39, 0.75)), url("/login-bg.png")',
      ...
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
          <div
            style={{
              ...
              color: '#ffffff',
              letterSpacing: '-0.5px',
              textShadow: '0 2px 16px rgba(0, 195, 255, 0.5)',
            }}
          >
            PrepHire<span style={{ color: T.gold }}>.</span>AI
          </div>
        ...
        <div
          style={{
            ...
            boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.4), 0 0 25px rgba(0, 195, 255, 0.15)',
          }}
        >
          {/* Tab toggle */}
          <div
...
function FacultyDashboard({
  ...
  const branchStudentEmails = new Set(
    users
      .filter((u) => u.role === 'Student' && u.department === facultyBranch)
      .map((u) => u.email.toLowerCase())
  )
  const branchSessions = allSessions.filter((s) =>
    s.department === facultyBranch || branchStudentEmails.has(s.studentEmail.toLowerCase())
  )
  ...
  const totalReviewed = branchSessions.filter((s) => s.facultyRemarks || s.placementReady !== undefined).length
  const totalStudentsCount = new Set(branchSessions.map(s => s.studentEmail)).size
  const avgScore =
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', minHeight: 'calc(100vh - 60px)' }}>
        ...
        >
          <div style={{ fontSize: 10, fontWeight: 700, color: T.txtMut, textTransform: 'uppercase', marginBottom: 4, letterSpacing: '0.08em' }}>
            Faculty Portal
          </div>
          <div style={{
            fontSize: 10,
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
                ...
                <GlassCard style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 24, fontWeight: 800, color: T.primary }}>{totalReviewed} / {branchSessions.length}</span>
                </GlassCard>
          ...
          {activeTab === 'ranking' && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              <Leaderboard
                ...
                userRole="faculty"
                placementReadyMap={Object.fromEntries(
                  branchSessions
                    .filter(s => s.placementReady !== undefined)
                    .map(s => [s.studentEmail.toLowerCase(), !!s.placementReady])
                )}
              />
            </div>
          )}
...
function AdminDashboard({
  ...
  const domainCounts = {
    ...
    'Group Discussion': allSessions.filter((s) => s.domain === 'Group Discussion').length,
  }
  ...
  async function handleDepartmentChange(userId: string, newDept: string) {
    await onDepartmentChange(userId, newDept)
  }
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchesBranch = branchFilter === 'All' || u.department === branchFilter
...
export default function App() {
  ...
  const [introTimeLimit, _setIntroTimeLimit] = useState(120)
  ...
  useEffect(() => {
    if (!profile) return
    setResumeProfile(profile.resumeProfile || null)
    setResumeNotes(profile.resumeNotes || '')
    setResumeFileName('')
  }, [profile])
  ...
  const generateReport = useCallback(async (data: QA[]) => {
    ...
    const domainObj = DOMAINS.find((d) => d.id === domain)
    const qAnswered = data.length
    const isEarlyEnd = qAnswered < 5
    ...
    if (qAnswered === 0) {
      r = {
        ...
        relevance: 0,
        strengths: [],
        improvements: ['No interview questions were answered.'],
...
{"overallScore":<0-100>,"technical":<0-100>,"communication":<0-100>,"confidence":<0-100>,"clarity":<0-100>,"relevance":<0-100>,"strengths":["...","...","..."],"improvements":["...","...","..."],"tip":"...","proctoringNote":"..."}`
      try {
        ...
        if (isEarlyEnd) {
          r.proctoringNote = (r.proctoringNote ? r.proctoringNote + ' ' : '') + `Interview ended early after ${qAnswered} of 5 questions.`
        }
      } catch {
        const avgScore = Math.round(data.reduce((sum) => sum + 50, 0) / data.length)
        r = {
          ...
          clarity: avgScore,
          relevance: avgScore,
          strengths: ['Interview was attempted.'],
          improvements: isEarlyEnd ? ['Interview was ended early. Try completing all 5 questions for a full evaluation.'] : [],
    ...
    // ── Upload video to Google Drive (skip if no questions answered) ──
    if (qAnswered > 0 && videoBlob && videoBlob.size > 0) {
      uploadVideoToDrive(videoBlob, sessionId).then((driveLink) => {
        if (!driveLink) return
        setAllSessions((prev) => prev.map((s) => (s.id === sessionId ? { ...s, driveLink } : s)))
        updateSession(sessionId, { driveLink }).catch((error) => {
          console.error('Failed to save drive link:', error)
          showToast('Failed to save drive link', 'error')
        })
      }).catch((error) => {
  ...
  async function handleResumeUpload(file: File) {
    ...
    try {
      ...
      if (!text) {
        throw new Error('No readable text found in this PDF. Try pasting your resume text instead.')
      }
      const parsed = await analyzeResume(text)
      setResumeProfile(parsed)
      setResumeFileName(file.name)
      showToast('Resume analyzed — questions will be tailored to your profile.')
      if (profile?.uid) {
  ...
  const handleProcWarn = useCallback((w: string) => {
    setProcWarnings((p) => {
      const newWarnings = [...p, { text: w, time: new Date().toLocaleTimeString() }]
      if (newWarnings.length >= 5) {
        showToast('Due to multiple breaches, you will have to restart the interview', 'error')
        setScreen('home')
        setTimerActive(false)
        setIntroTimerActive(false)
      }
  ...
  async function sendAnswer(ans: string) {
    ...
    setSessionData(sd)
    const domainObj = DOMAINS.find((d) => d.id === domain)
    const nextQIdx = getNextQuestionIndex(domainObj?.questions || [], usedQuestionIndices)
    setUsedQuestionIndices((prev) => [...prev, nextQIdx])
    setLoading(true)
            ...
            {studentTab === 'interview' ? (
              ...
              <div className="home-screen-content" style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px 88px 24px', animation: 'fadeIn 0.4s ease' }}>
                ...
                >
                  ...
                  <p className="text-body" style={{ color: T.txtSec }}>
                    Manage your details and resume. Your resume is analyzed once and reused for every Technical & HR interview.
                  </p>
                </section>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 16, alignItems: 'start' }}>
                  ...
                  <GlassCard style={{ marginBottom: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <FileText size={15} style={{ color: T.primary, flexShrink: 0 }} />
                      <span style={{ fontSize: 14, fontWeight: 700, color: T.txtPri }}>Resume</span>
                      <span style={{ fontSize: 11, color: T.txtMut }}>— analyzed once by AI, stored here for every interview</span>
                    </div>
                    <div style={{ fontSize: 11, color: T.txtMut, marginBottom: 12 }}>
                      Used for Technical & HR interviews. The AI asks questions grounded in your skills, projects & tech stack.
                    </div>

                    {resumeLoading ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13, color: T.txtSec, padding: '16px 0' }}>
                        <span style={{ display: 'inline-block', width: 14, height: 14, border: `2px solid ${T.border}`, borderTopColor: T.primary, borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                        Analyzing your resume…
                      </div>
                    ) : resumeProfile ? (
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                          <Pill color={T.green} bg={T.greenBg}>{resumeFileName || 'Resume loaded'}</Pill>
                          {resumeProfile.techStacks.slice(0, 4).map((s, i) => (
                            <Pill key={`t${i}`}>{s}</Pill>
                          ))}
                          {resumeProfile.skills.slice(0, 4).map((s, i) => (
                            <Pill key={`s${i}`}>{s}</Pill>
                          ))}
                          <Btn variant="ghost" size="sm" onClick={handleResumeRemove} style={{ marginLeft: 'auto' }}>
                            <Trash2 size={12} /> Remove
                          </Btn>
                        </div>
                        {resumeProfile.projects.length > 0 && (
                          <div style={{ marginTop: 12, fontSize: 12, color: T.txtSec, lineHeight: 1.5 }}>
                            <span style={{ fontWeight: 700, color: T.txtPri }}>Projects:</span> {resumeProfile.projects.join(' · ')}
                          </div>
                        )}
                        {resumeProfile.certifications.length > 0 && (
                          <div style={{ marginTop: 6, fontSize: 12, color: T.txtSec, lineHeight: 1.5 }}>
                            <span style={{ fontWeight: 700, color: T.txtPri }}>Certifications:</span> {resumeProfile.certifications.join(' · ')}
                          </div>
                        )}
                        <label style={{ display: 'inline-block', marginTop: 12, cursor: 'pointer', fontSize: 12, color: T.primary, fontWeight: 600 }}>
                          Replace resume
                          <input type="file" accept=".pdf" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) handleResumeUpload(f); e.target.value = '' }} />
                        </label>
                      </div>
                    ) : (
                      <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, border: `1.5px dashed ${T.border}`, borderRadius: 10, padding: 20, cursor: 'pointer', background: T.bg }}>
                        <UploadCloud size={18} style={{ color: T.primary }} />
                        <span style={{ fontSize: 13, color: T.txtSec, fontWeight: 600 }}>Upload PDF resume</span>
                        <input type="file" accept=".pdf" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) handleResumeUpload(f); e.target.value = '' }} />
                      </label>
                    )}

                    {resumeError && (
                      <div style={{ fontSize: 12, color: T.error, marginTop: 8 }}>{resumeError}</div>
                    )}

                    <textarea
                      value={resumeNotes}
                      onChange={(e) => setResumeNotes(e.target.value)}
                      placeholder="Anything else you'd like the AI to know? (optional) — target role, dream company, weak areas you want to practice…"
                      style={{
                        width: '100%',
                        height: 60,
                        border: `1px solid ${T.border}`,
                    ...
                    />
                    {resumeNotes.trim() && (
                      <div style={{ fontSize: 11, color: T.txtMut, marginTop: 6 }}>
                        Saved automatically when you start an interview.
                      </div>
                    )}
                  </GlassCard>
                </div>
  ...
  if (screen === 'intro') {
    ...
    const ip = Math.round((introTimer / introTimeLimit) * 100)
        ...
        <div style={{ maxWidth: 1000, margin: '0 auto', padding: '2rem 1rem', animation: 'fadeIn 0.4s ease' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 200px', gap: 14, alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {resumeProfile ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: T.greenBg, borderRadius: 10, padding: '8px 14px', fontSize: 12, color: T.green, fontWeight: 600 }}>
                  <FileText size={13} style={{ flexShrink: 0 }} />
                  Resume profile loaded — {activeDomain?.id === 'technical' || activeDomain?.id === 'hr' ? 'questions will be tailored to your skills & tech stack.' : 'used for Technical & HR interviews.'}
                  <span style={{ marginLeft: 'auto', fontWeight: 400, fontSize: 11 }}>
                    Manage in <strong>Profile</strong>
                  </span>
                </div>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: T.amberBg, borderRadius: 10, padding: '8px 14px', fontSize: 12, color: T.amber, fontWeight: 600 }}>
                  <FileText size={13} style={{ flexShrink: 0 }} />
                  Tip: upload your resume in <strong>Profile</strong> for AI questions based on your skills & tech stack.
                </div>
              )}
              ...
              <div style={{ background: T.bgWhite, borderRadius: 10, padding: '8px 14px', border: `1px solid ${T.border}` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 11, color: im, fontWeight: 700 }}>{introTimeLimit - introTimer}s / {introTimeLimit}s</span>
                </div>
                <div style={{ height: 4, background: T.bgHigh, borderRadius: 4, overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: `${((introTimeLimit - introTimer) / introTimeLimit) * 100}%`, background: im, borderRadius: 4, transition: 'width 1s linear' }} />
                </div>
              </div>
            </div>
    ...
    <div style={{ minHeight: '100vh', background: T.bg }}>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '1.5rem 1rem', animation: 'fadeIn 0.3s ease' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Btn onClick={() => setShowEndConfirm(true)} variant="ghost" size="sm" style={{ color: T.error, borderColor: T.error }}>
      ...
      {showEndConfirm && (
        ...
        >
          ...
          >
            <AlertTriangle size={36} style={{ color: T.amber, margin: '0 auto' }} />
            <div>
              <div style={{ fontSize: 17, fontWeight: 700, color: T.primary, marginBottom: 6 }}>
                End Interview?
              </div>
              <p style={{ fontSize: 13, color: T.txtSec, lineHeight: 1.5, margin: 0 }}>
                {qCount === 0
                  ? 'You haven\'t answered any questions yet. Ending now will result in a score of 0.'
                  : qCount < 5
                    ? `You've answered ${qCount} of 5 questions. Ending early will grade only the questions completed.`
                    : 'You\'ve completed all questions. Proceed to your report.'}
              </p>
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <Btn onClick={() => setShowEndConfirm(false)} variant="outline">
                Continue Interview
              </Btn>
              <Btn
                onClick={() => { setShowEndConfirm(false); generateReport(sessionData) }}
                variant="navy"
                style={{ background: T.error, border: 'none' }}
              >
                End Session
              </Btn>
            </div>
